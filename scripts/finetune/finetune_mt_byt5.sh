...
#SBATCH --job-name="ft-mt-byt5"
#SBATCH --mail-user=rmdrak003@myuct.ac.za
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output=logs/finetune_%x_%j.log
#SBATCH --error=logs/finetune_%x_%j.log
#
# FULL sweep: byt5 on mt, all 20 checkpoints (base + 19) in one
# job, sequentially. Resume-safe (done runs skipped via results.jsonl),
# so resubmitting after a 48h timeout continues. Chain: sbatch
# --export=ALL,CHAIN_JOBS=N <this>.
...
# W&B on by default (live curves, grouped byt5-mt); WANDB=0 disables.
WANDB_FLAG=""
...
echo "=========================================================="
echo "finetune ${MODEL} on ${TASK}  (FULL 20 ckpts)   start $(date -Is)"
echo "=========================================================="
