...
#SBATCH --job-name="lafand-pad-faithful"
#SBATCH --mail-user=rmdrak003@myuct.ac.za
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output=logs/lafand_pad_faithful_%j.log
#SBATCH --error=logs/lafand_pad_faithful_%j.log
#
# Padding A/B test (pad_token_id in labels, lafand-faithful): decides whether the lafand pipeline keeps
...
uv run accelerate launch \
    ...
    --eval-steps 15 \
    --n-eval-obs 2000 \
    --wandb-run-name t5-lafand-pad-faithful \
    --metrics-filename metrics_lafand_pad_faithful.jsonl \
    --run-subdir lafand-pad-faithful \
    --no-save
