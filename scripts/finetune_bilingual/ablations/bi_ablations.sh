...
#SBATCH --job-name="bil-ablation"
#SBATCH --mail-user=rmdrak003@myuct.ac.za
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output=logs/bil_ablation_%x_%j.log
#SBATCH --error=logs/bil_ablation_%x_%j.log

set -e

MODEL="${1:?usage: sbatch bi_ablations.sh <t5|byt5|nguni-byt5> <config basename>}"
CFG="${2:?usage: sbatch bi_ablations.sh <t5|byt5|nguni-byt5> <config basename>}"

case "${MODEL}" in
  t5|byt5|nguni-byt5) CKPTS="/scratch/rmdrak003/results/${MODEL}/lafand-bilingual/checkpoints" ;;
...
echo "bilingual ablation ${CFG} : ${MODEL}  start $(date -Is) "
