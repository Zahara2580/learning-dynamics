...
#SBATCH --job-name="cpt-byt5-bs8"
#SBATCH --mail-user=rmdrak003@myuct.ac.za
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output=logs/pretrain_byt5_bs8_%j.log
#SBATCH --error=logs/pretrain_byt5_bs8_%j.log
#
# Production batch-size CANDIDATE for byt5: per_device_batch_size=8,
# gradient_accumulation_steps=128 (effective batch size 1024).
# Real full run (total_steps/warmup_steps from configs/models/byt5.yaml, no --max-steps
...
#
# Methodology: this is one of several batch-size candidates for byt5,
# each launched as its own real run under run-subdir "byt5-xho-bs8" so
...
uv run accelerate launch \
    ...
    --wandb-run-name byt5-xho-bs8 \
    --metrics-filename metrics_byt5-xho-bs8.jsonl \
    --run-subdir byt5-xho-bs8 \
    --eval-steps 200 \
    --save-steps 200 \
    --resume
