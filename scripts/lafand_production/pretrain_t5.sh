...
#SBATCH --job-name="cpt-lafand-t5"
#SBATCH --mail-user=rmdrak003@myuct.ac.za
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --output=logs/lafand_prod_t5_%j.log
#SBATCH --error=logs/lafand_prod_t5_%j.log
#
# PRODUCTION lafand-pipeline CPT for t5: full 10k steps / 5000 warmup /
...
#
# BEFORE FIRST SUBMISSION: confirm batch-size 16/accum 64 against the
...
#
# Chains additional 24h jobs via CHAIN_JOBS (default 2 resubmissions).

# Update to latest commit
git pull
git log -1

CHAIN_JOBS=${CHAIN_JOBS:-2}
...
uv run accelerate launch \
    ...
    --eval-steps 200 \
    --n-eval-obs 2000 \
    --wandb-run-name t5-xho-lafand \
    --metrics-filename metrics_t5_lafand.jsonl \
